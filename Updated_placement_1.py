...
def overlap_repulsion_loss(cell_features, pin_features, edge_list):
    ...
    deltas = pos_i - pos_j
    abs_deltas_x = torch.abs(deltas[:, :, 0])
    abs_deltas_y = torch.abs(deltas[:, :, 1])
    ...
    overlap_x = torch.relu(min_sep_x - abs_deltas_x)
    overlap_y = torch.relu(min_sep_y - abs_deltas_y)
    ...
    # Apply square to each individual overlap, not the sum
    # This creates stronger repulsion for each overlapping pair
    overlap_penalties = overlap_areas ** 2
    total_overlap = torch.sum(overlap_penalties)
...
def train_placement(
    ...
    pin_features,
    edge_list,
    num_epochs=4000,        # Increased further
    lr=0.08,                # Increased learning rate
    lambda_wirelength=1.0,
    lambda_overlap=150.0,   # Increased penalty
    verbose=True,
    log_interval=100,
):
